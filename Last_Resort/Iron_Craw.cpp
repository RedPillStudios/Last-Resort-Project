...
update_status Iron_Craw::Update() {
	if (!dead) {
		...
		App->render->Blit(Mini_Boss, position.x + 38, position.y, &Right_Arm.GetCurrentFrame());
		App->render->Blit(Mini_Boss, position.x - 44, position.y, &Left_Arm.GetCurrentFrame());
